# management/commands/update_onepiece_prices.py
# # 원피스카드 기본 실행
# python manage.py update_onepiece_prices

# # 특정 날짜로 업데이트
# python manage.py update_onepiece_prices --date 2024-12-01

# # 특정 카드만 업데이트
# python manage.py update_onepiece_prices --card-id 123

# # 제한된 수만 업데이트 (테스트용)
# python manage.py update_onepiece_prices --limit 10

# # 가격 비교 시뮬레이션 (실제 업데이트 없이)
# python manage.py update_onepiece_prices --dry-run --limit 10

# # 이미 데이터가 있어도 강제로 업데이트
# python manage.py update_onepiece_prices --force

# # 옵션 조합
# python manage.py update_onepiece_prices --date 2024-12-01 --limit 5 --dry-run
...
import time
from cardStockManageApp.services.onepiece_price_service import OnepiecePriceService


class Command(BaseCommand):
    help = '원피스카드 일별 최저가 업데이트'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'원피스카드 가격 업데이트 시작: {start_time}')
        )
    ...
    def _dry_run_single_card(self, service, card_version):
        ...
        search_keyword, rarity, card_number = service.extract_onepiece_card_info(card_version)
        
        if search_keyword == "EXCLUDE":
            self.stdout.write(f'카드: {card_name}')
            self.stdout.write(f'상태: SP- 레어도로 제외됨')
            return
        ...
        try:
            ...
            if items:
                min_price, valid_results = service.filter_onepiece_api_results(
                    items, search_keyword, card_number, debug=True
                )
                if min_price:
    ...
    def _dry_run_all_cards(self, service, limit):
        ...
        if limit:
            onepiece_cards = onepiece_cards[:limit]
        
        count = onepiece_cards.count()
        self.stdout.write(f'업데이트 대상: {count}개 카드')
        self.stdout.write('현재가격 → 예상최저가 형태로 표시됩니다...\n')
        
        for idx, card in enumerate(onepiece_cards, 1):
            card_name = card.card.name_kr or card.card.name
            search_keyword, rarity, card_number = service.extract_onepiece_card_info(card)
            
            if search_keyword == "EXCLUDE":
                self.stdout.write(f'  [{idx:2d}] {card_name} | SP- 레어도 제외 | -')
                continue
            ...
            try:
                ...
                if items:
                    debug_mode = (idx == 1)
                    min_price, valid_results = service.filter_onepiece_api_results(
                        items, search_keyword, card_number, debug=debug_mode
                    )
                    if min_price:
                        adjusted_price = int(min_price + service.plus_price)
    ...
    def _update_single_card(self, service, card_version, target_date, force):
        ...
        result = service.update_onepiece_card_daily_price(card_version, target_date)
    ...
    def _update_all_cards(self, service, target_date, limit, force):
        ...
        if force:
            ...
            deleted_count = DailyPriceHistory.objects.filter(
                card_version__card__game__name_kr="원피스",
                date=target_date
            ).delete()[0]
        ...
        if limit:
            ...
            results = service.update_all_onepiece_cards_daily(target_date)
            self.stdout.write(
                self.style.SUCCESS(
