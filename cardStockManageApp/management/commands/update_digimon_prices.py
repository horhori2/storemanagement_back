# management/commands/update_digimon_prices.py
# # 디지몬카드 기본 실행
# python manage.py update_digimon_prices

# # 특정 날짜로 업데이트
# python manage.py update_digimon_prices --date 2024-12-01

# # 특정 카드만 업데이트
# python manage.py update_digimon_prices --card-id 123

# # 제한된 수만 업데이트 (테스트용)
# python manage.py update_digimon_prices --limit 10

# # 가격 비교 시뮬레이션
# python manage.py update_digimon_prices --dry-run --limit 10

# # 강제 업데이트
# python manage.py update_digimon_prices --force

from django.core.management.base import BaseCommand
from django.utils import timezone
from datetime import datetime
import time
from cardStockManageApp.services.digimon_price_service import DigimonPriceService


class Command(BaseCommand):
    help = '디지몬카드 일별 최저가 업데이트'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'디지몬카드 가격 업데이트 시작: {start_time}')
        )
    ...
    def _dry_run_single_card(self, service, card_version):
        ...
        search_keyword, rarity, card_number = service.extract_digimon_card_info(card_version)
        ...
        try:
            ...
            if items:
                min_price, valid_results = service.filter_digimon_api_results(
    ...
    def _dry_run_all_cards(self, service, limit):
        ...
        if limit:
            digimon_cards = digimon_cards[:limit]
        
        count = digimon_cards.count()
        self.stdout.write(f'업데이트 대상: {count}개 카드')
        self.stdout.write('현재가격 → 예상최저가 형태로 표시됩니다...\n')
        
        for idx, card in enumerate(digimon_cards, 1):
            card_name = card.card.name_kr or card.card.name
            search_keyword, rarity, card_number = service.extract_digimon_card_info(card)
    ...
    def _update_single_card(self, service, card_version, target_date, force):
        ...
        result = service.update_digimon_card_daily_price(card_version, target_date)
    ...
    def _update_all_cards(self, service, target_date, limit, force):
        """모든 카드 업데이트"""
        if force:
            from ycardStockManageAppour_app.models import DailyPriceHistory
            deleted_count = DailyPriceHistory.objects.filter(
                card_version__card__game__name_kr="디지몬",
                date=target_date
            ).delete()[0]
        ...
        if limit:
            ...
            results = service.update_all_digimon_cards_daily(target_date)
            self.stdout.write(
                self.style.SUCCESS(
